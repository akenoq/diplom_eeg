...
def lets_start():
    ...
    while True:  # Infinite loop
        ...
        if FSM_state == 0:  # Education
            ...
            for i in range(3):
                # show image
                # GPIO.output(way_l, GPIO.HIGH)
                # GPIO.output(way_r, GPIO.LOW)
                ...
                # show image
                # GPIO.output(way_l, GPIO.LOW)
                # GPIO.output(way_r, GPIO.HIGH)
            ...
            if recognition[0] - recognition[1] >= 0.5:
                ...
                time.sleep(2)
                write_pack_fifo_buf_3D("W__")
            elif recognition[1] - recognition[0] >= 0.5:
                print "==>"
                # пишем направление в именованный канал
            ...
            # если переключатель1 в "старт"
            # if GPIO.input(btn_start) == False:
            #     if recognition[0] - recognition[1] >= 0.5:
            #         print "<=="
            #         GPIO.output(motor_l, GPIO.HIGH)
            #         GPIO.output(motor_r, GPIO.LOW)
            #         # если переключатель2 в 3D
            #         if GPIO.input(btn_3d) == False:
            #             # пишем направление в именованный канал
            #             write_pack_fifo_buf_3D("WD__")
            #             time.sleep(2)
            #             write_pack_fifo_buf_3D("W__")
